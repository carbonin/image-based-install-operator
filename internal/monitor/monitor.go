...
func (status *ClusterInstallStatus) String() string {
	...
	return fmt.Sprintf("Cluster is %s, ClusterVersion Status: %s, Nodes Status: %s", installStatus, status.ClusterVersionStatus, status.NodesStatus)
}
	...
	for _, cond := range cv.Status.Conditions {
		if cond.Type == configv1.OperatorAvailable {
			if cond.Status == configv1.ConditionTrue {
				return true, "ClusterVersion is available", nil
			} else {
